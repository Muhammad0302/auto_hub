...
const Login = () => {
	...
	const [isNewUser, setIsNewUser] = useState(() => {
		const storedValue = localStorage.getItem('newUser');
		return storedValue !== null ? JSON.parse(storedValue) : 0;
	});
	const [packageId, setPackageId] = useState(() => {
		const storedValue = localStorage.getItem('packageId');
				...
				fetch(`${baseURL}/register`, {
					...
					.then(async (data) => {
						...
						if (data?.status === 'ok') {
							...
							navigate(`/Auto_hub/auth-pages/login`, { replace: true });
							setLastSave(moment());
							setIsNewUser(0);
						} else {
				...
				fetch(`${baseURL}/login`, {
					...
					.then(async (data) => {
						...
						if (data?.status === 'ok') {
							...
							Cookies.set('role_name', data.role.role);
							Cookies.set('role_id', data.role.user.role.id);
							Cookies.set('id', data.role.id);
							Cookies.set('last_login', data.role.last_login);
							Cookies.set('Data', JSON.stringify(data.role));
							...
							<Card className='shadow-3d-dark' data-tour='login-page'>
								<CardBody>
									...
									<form className='row g-4'>
										...
										<div className='col-12'>
											<Link
												to='/Auto_hub/auth-pages/get-started'
