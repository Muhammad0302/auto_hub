...
const validate = (values) => {
	...
	values.list.forEach((data, index) => {
		// data.item_racks.forEach((item, rack) => {
		// 	if (item.checked) {
		// 		// Sum up the quantities of item_racks where check is true
		// 		const totalQuantity = item.item_racks.reduce(
		// 			(sum, rack1) => sum + rack1.quantity,
		// 			0,
		// 		);

		// 		if (item.qty > totalQuantity) {
		// 			// Update errors if qty is greater than the sum of quantities
		// 			errors = {
		// 				...errors,
		// 				[`list[${index}].qty`]:
		// 					'Should not be greater than the sum of quantities of item_racks',
		// 			};
		// 			console.log('greater');
		// 		}
		// 	}
	...
	const getItemsBasedOnSubCategory = (index) => {
		...
		apiClient
			...
			.then((response) => {
				...
				formik.setFieldValue(`list[${index}].items_options`, rec1);
				formik.setFieldValue(`list1[${index}].items_options`, response.data.data);
				...
				// Call function here
				const selectedVal = formik.values.list[index].selectedValue; // Assuming you have a field to store the selected value
				// getRacksAndShelfData(selectedVal, index);
				getBrands(index);
			})
			.catch((err) => {});
	};
		...
		<div className='col-auto'>
			...
			<Modal
				...
				<ModalHeader setIsOpen={headerCloseStatus ? setState : null}>
					<CardLabel icon='Add'>
						<ModalTitle id='exampleModalLabel'>Add Sale Order ifnelifmei</ModalTitle>
					</CardLabel>
				</ModalHeader>
				<ModalBody>
										...
										{formik.values.list.length > 0 &&
											formik.values.list.map((items, index) => (
												<tr className='row' key={items.index}>
													<td
														className='col-md-5 border-start border-end '
														style={{
															padding: 0,
														}}>
														<FormGroup
													...
													<td className='col-md-1 mt-1 border-start border-end '>
