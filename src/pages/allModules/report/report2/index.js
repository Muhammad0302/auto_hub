...
const Categories = () => {
	...
	const printReportAll = (docType) => {
		...
		apiClient
			.get(
				`/getSalesReport?records=${store.data.reportManagementModule.report1.perPage}&pageNo=${store.data.reportManagementModule.report1.pageNo}&colName=id&sort=desc&sale_type=${formik.values.sale_type}&customer_id=${formik.values.customer_id}&walk_in_customer_name=${formik.values.walk_in_customer_name}&store_id=${formik.values.store_id}&item_id=${formik.values.machine_part_id}&from=${startDate3}&to=${endDate3}`,
			)
			.then((response) => {
